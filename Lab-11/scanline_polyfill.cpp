...
// Calculates necessary values from two vertexes.
Edges::Edges(tuple<GLint, GLint> vertexOne, tuple<GLint, GLint> vertexTwo)
{
    ...
    this->slope =
        static_cast<GLfloat>(static_cast<GLfloat>(get<1>(vertexOne) - get<1>(vertexTwo))) / static_cast<GLfloat>((get<0>(vertexOne) - get<0>(vertexTwo)));
}
...
void drawPixels(GLfloat x1, GLfloat x2)
{
    ...
    for (int i = ((1200 * (740 - scanline)) + i1); i < ((1200 * (740 - scanline)) + i2); i++, count++)
    {
        pixels[i].r = 0;
        pixels[i].b = 1;
        pixels[i].g = 0;
...
void init(void)
{
    ...
    glLoadIdentity();
    glOrtho(0.0, 1200, 740, 0.0, 0.0, 1.0);

    //Initialize RGB array
    pixels = new RGBType[1200 * 1200];
...
void mouse_down(int button, int state, int x, int y)
{
    ...
    case GLUT_LEFT_BUTTON:
        ...
        {
            // printf("ADDING POINT %d and %d", x, 740 - y);
...
void display(void)
{
    ...
    glDrawPixels(1200, 740, GL_RGB, GL_FLOAT, pixels);
    glPointSize(5);
    glColor3f(1, 0.0, 0);
